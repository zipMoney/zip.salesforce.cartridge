...
var ZipModel = require('int_zippay_sfra/cartridge/scripts/zip/helpers/zip');
var ZipOrderModel = require('int_zippay_sfra/cartridge/models/zip/zipOrder');
...
function capture() {
    ...
    var captureResult = hooksHelper('zippay.order.processor',
        'Capture', orderNo, amount, require('int_zippay_sfra/cartridge/scripts/hooks/zip/orderprocess').Capture
    );
    var responseResult = captureResult.success ? 'Success' : 'Error';

    renderJson(responseResult, captureResult.result);
}
...
function refund() {
    ...
    var refundResult = hooksHelper('zippay.order.processor',
       'Refund', orderNo, amount, reason, require('int_zippay_sfra/cartridge/scripts/hooks/zip/orderprocess').Refund
    );
    var responseResult = refundResult.success ? 'Success' : 'Error';

    renderJson(responseResult, refundResult.result);
}

/**
 * Cancel order.
 *
 * @param {Object} req instance of request object.
 * @param {Object} res instance of response object.
 */
function cancel(req, res) {
    ...
    if (empty(orderNo)) {
        res.print(JSON.stringify({ msg: 'Order not found' }));
        return;
    }

    var result = hooksHelper('zippay.order.processor',
       'Cancel', orderNo, require('int_zippay_sfra/cartridge/scripts/hooks/zip/orderprocess').Cancel
    );
